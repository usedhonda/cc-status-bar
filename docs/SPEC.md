...
### 22.6 Codex Session Support

Session display mode applies to both Claude Code and Codex sessions:
- `createCodexSessionMenuItem()` uses `codexSession.displayText(for:)`
- `PinnedCodexSessionRowView` uses `codexSession.displayText(for:)`

### 22.7 Implementation

- **File**: `Sources/Models/Session.swift`
- **Method**: `Session.displayText(for:paneInfo:)` (shared logic for all views)
- **File**: `Sources/Models/CodexSession.swift`
- **Method**: `CodexSession.displayText(for:)` (shared logic for Codex views)
- **File**: `Sources/Services/AppSettings.swift`
- **Enum**: `SessionDisplayMode`
- **Property**: `AppSettings.sessionDisplayMode`
- **File**: `Sources/Services/TmuxHelper.swift`
- **Struct**: `PaneInfo` (extended with `windowName`, default value `""`)
- **File**: `Sources/App/AppDelegate.swift`
- **Method**: `createSessionMenuItem(_:)`, `createCodexSessionMenuItem(_:)`, `createSessionDisplayMenu()`
- **File**: `Sources/Views/SessionListView.swift`
- **Computed Property**: `SessionRowView.displayText` (delegates to `Session.displayText`)
- **File**: `Sources/Views/SessionListWindow.swift`
- **Computed Property**: `PinnedSessionRowView.displayText`, `PinnedCodexSessionRowView.displayText`
