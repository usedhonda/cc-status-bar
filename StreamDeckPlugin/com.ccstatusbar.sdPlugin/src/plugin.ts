 ...
 */
function buildAutoRank(): Map<string, number> {
    ...
    autoButtons.sort((a, b) => {
        ...
        if (infoA.row !== infoB.row) return infoA.row - infoB.row;
 ...
 */
function updateSessionButtonWithIndex(
    context: string,
 ...
 */
function createSessionButtonSVG(projectName: string, bgColor: string, sessionIndex: number = 0, iconBase64?: string): string {
